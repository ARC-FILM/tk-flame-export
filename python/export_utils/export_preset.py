...
class ExportPreset(object):
    ...
    def batch_quicktime_path_from_render_path(self, batch_render_path):
        ...
        if self.get_batch_quicktime_template() is None:
            raise TankError(
                "%s: Cannot evaluate quicktime path because no "
    ...
    def get_xml_path(self):
        ...
        self._app.log_debug(
            "Flame preset generation: Setting version padding to %s based on "
    ...
    def __write_content_to_file(self, content, file_name):
        ...
        file_path = os.path.join(
            self._app.cache_location, self._app.instance_name, file_name
        )
    ...
    def __resolve_flame_templates(self):
        ...
        template_defs["shot_clip_template"] = self._app.get_template(
            "shot_clip_template"
        ).definition
        template_defs["segment_clip_template"] = self._app.get_template(
        ...
        for t in template_defs:
            ...
            template_defs[t] = template_defs[t].replace(
                "{%s}" % shot_parent_entity_type, "<name>"
            )
...
class ExportPresetHandler(object):
    ...
    def __init__(self):
        ...
        self._app.log_debug(
            "ExportPresetHandler loaded export preset data "
    ...
    def get_preset_by_name(self, preset_name):
        ...
        if preset_name not in self._export_presets:
            raise TankError(
                "Export preset manager cannot find preset '%s' in the configuration!"
                % preset_name
            )
    ...
    def get_preset_for_batch_render_path(self, path):
        ...
        self._app.log_debug(
            "Trying to locate an export preset " "for batch render path '%s'..." % path
        )
